...
{
    double x = to_real(v[0].to_string());
    double fitness = (x-1.)*(x-1.);

    for(int i = 1; i < N; i++)
    {
        double y = to_real(v[i-1].to_string());
        fitness += ((i+1)*(2.*y*y-x)*(2.*y*y-x));
    }
            ...
            for(int p = 0; p < N; p++)
            {
                ...
                while(!local)
                {
                    ...
                    while(j < left->size() || j < right->size())
                    {
                        ...
                        {
                            bitset<BITS> b(left->at(j));
                            aux[p] = b;
                            if(evaluate(aux) < fitness)
                            {
                                found = true;
                                bttr_string = left->at(j);
                                fitness = evaluate(aux);
                                break;
                            }
                        }
        ...
        while(temperature > EPSILON)
        {
            for(int it = 0; it < ITERATIONS/100; it++)
            {
                for(int p = 0; p < N; p++)
                {
                    ...
                    vector<bitset<BITS>> aux(x_curr);
                    bitset<BITS> b(nghbr_string);
                    aux[p] = b;

                    double nghbr_fitness = evaluate(aux);
...
void run()
{
    ...
    {
        ITERATIONS = 1000;
        out.open("dixonprice_NAHC.txt", ios::out | ios::app);
        NAHC();
    ...
    if(O == 2)
    {
        out.open("dixonprice_SAHC.txt", ios::out | ios::app);
        SAHC();
        avg_time = Clock::now() - clk;
        display();
        return;
    ...
    {
        ITERATIONS = 10000;
        out.open("dixonprice_SA.txt", ios::out | ios::app);
        SA();
        avg_time = Clock::now() - clk;
        display();
        return;
