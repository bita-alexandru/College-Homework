...
{
    double fitness = bin_to_real(gray_to_bin(v[0].to_string())) - 1;
    fitness *= fitness;

    for(int i = 1; i < N; i++)
    {
        double x = bin_to_real(gray_to_bin(v[i-1].to_string()));
        double y = bin_to_real(gray_to_bin(v[i].to_string()));
...
{
    clk = Clock::now();
    out.open("dixonprice.txt", ios::out | ios::app);
