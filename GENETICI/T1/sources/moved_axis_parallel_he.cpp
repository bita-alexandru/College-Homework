        ...
        for(int it = 0; it < ITERATIONS; it++)
        {
            ...
            for(int p = 0; p < N; p++)
            {
                ...
                while(!local)
                {
                    ...
                    string bttr_string;
                    double new_fitness = fitness - FUNC(p,curr_number);
                    ...
                    while(j < left->size() || j < right->size())
                    {
                        if(keep_left && j < left->size()) // left neighbours available
                        {
                            ...
                            if(new_fitness + FUNC(p,number) < fitness)
                            {
                                found = true;
                                keep_right = false;
                                bttr_string = left->at(j);
                                fitness = new_fitness + FUNC(p,number);
                                break;
                            }
                        }
        ...
        while(temperature > EPSILON)
        {
            for(int it = 0; it < ITERATIONS; it++)
            {
                for(int p = 0; p < N; p++)
                {
                    ...
                    double nghbr_fitness = curr_fitness - FUNC(p,curr_number) + FUNC(p,nghbr_number);
...
void init()
{
    ...
    while(true)
    {
        ...
        neighbour = curr;
        for(int i = 0; i < MAX_NGHBRS; i++) // left neighbours
            if(neighbour == LOWEST.to_string()) break;
            else
            {
        ...
        neighbour = curr;
        for(int i = 0; i < MAX_NGHBRS; i++) // right neighbours
            if(neighbour == HIGHEST.to_string()) break;
            else
            {
...
void run()
{
    ...
    if(O == 1)
    {
        out.open("movedaxis_NAHC.txt", ios::out | ios::app);
        NAHC();
    ...
    if(O == 2)
    {
        out.open("movedaxis_SAHC.txt", ios::out | ios::app);
        SAHC();
        avg_time = Clock::now() - clk;
        display();
        return;
    ...
    if(O == 3)
    {
        out.open("movedaxis_SA.txt", ios::out | ios::app);
        SA();
        avg_time = Clock::now() - clk;
        display();
        return;
